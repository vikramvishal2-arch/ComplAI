...
import { DAST_FINDINGS, VULNERABILITY_SEVERITY_STYLES } from '@/lib/data/assurance-demo';
...
export default function ApplicationDastPage() {
  ...
  const findings = useMemo(() => {
    if (filter === 'all') return DAST_FINDINGS;
    return DAST_FINDINGS.filter((f) => f.status === filter);
  }, [filter]);

  return (
    <AppShell
      title="Application DAST"
      subtitle="Dynamic application security testing — web apps and APIs scanned for exploitable flaws"
    >
      <AssuranceSubNav />
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-semibold text-slate-700">Application</th>
              <th className="px-4 py-3 text-left font-semibold text-slate-700">OWASP</th>
              ...
              <th className="px-4 py-3 text-left font-semibold text-slate-700">Detected</th>
            ...
            {findings.map((finding) => (
              <tr key={finding.id} className="hover:bg-slate-50/80">
                ...
                <td className="px-4 py-3">
                  ...
                  <p className="mt-0.5 max-w-xs truncate font-mono text-xs text-slate-500">
                    {finding.url}
                  </p>
                </td>
                <td className="px-4 py-3 text-slate-700">{finding.application}</td>
                <td className="px-4 py-3 text-xs text-slate-600">{finding.owaspCategory}</td>
                <td className="px-4 py-3">
                  <span
                    className={cn(
                ...
                <td className="px-4 py-3 text-slate-600">{finding.detectedAt}</td>
