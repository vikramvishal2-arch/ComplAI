...
import {
  ComplAIBrandLink,
  ComplAIText,
} from '@/components/marketing/complai-brand-link';
import {
...
export function WhyComplaiPageContent() {
  return (
    <>
      <section className="bg-scrut-hero py-16 text-white sm:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-scrut-teal">
            <ComplAIText>{WHY_COMPLAI_HERO.eyebrow}</ComplAIText>
          </p>
          <h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">
            {WHY_COMPLAI_HERO.title}
          </h1>
          <p className="mt-4 max-w-2xl text-lg text-white/75">
            <ComplAIText>{WHY_COMPLAI_HERO.subtitle}</ComplAIText>
          </p>
          ...
          <div className="grid gap-10 sm:grid-cols-3">
            {WHY_COMPLAI_OUTCOMES.map((outcome) => (
              <div key={outcome.label} className="text-center sm:text-left">
                ...
                <p className="mt-2 text-lg font-semibold text-zinc-100">{outcome.label}</p>
                <p className="mt-2 text-sm leading-relaxed text-zinc-400">{outcome.detail}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="differentiators" className="scroll-mt-24 border-y border-white/10 bg-marketing-surface-alt py-16 sm:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-3xl font-bold tracking-tight text-zinc-100 sm:text-4xl">
              What makes <ComplAIBrandLink inheritWeight /> different
            </h2>
            <p className="mt-4 text-lg text-zinc-400">
              {ORGANIZATION_NAME} built <ComplAIBrandLink inheritWeight /> for teams who need more
              than a checklist — they need a program that scales.
            </p>
          </div>
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2">
            {WHY_COMPLAI_DIFFERENTIATORS.map((item) => (
              <div
                key={item.id}
                id={item.id}
                className="scroll-mt-24 rounded-2xl border border-white/10 bg-scrut-navy-light/70 p-6 sm:p-8"
              >
                <h3 className="text-lg font-bold text-zinc-100">{item.title}</h3>
                <p className="mt-3 text-sm leading-relaxed text-zinc-400">
                  <ComplAIText>{item.description}</ComplAIText>
                </p>
              </div>
            ))}
          </div>
      ...
      <section id="comparison" className="scroll-mt-24 py-16 sm:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-3xl font-bold tracking-tight text-zinc-100 sm:text-4xl">
              Manual GRC vs. <ComplAIBrandLink inheritWeight />
            </h2>
            <p className="mt-4 text-lg text-zinc-400">
          ...
          <div className="mt-12 overflow-hidden rounded-2xl border border-white/10">
            <div className="grid grid-cols-[1fr_auto_1fr] bg-scrut-navy-light/50 text-sm font-semibold text-zinc-100">
              <div className="px-4 py-3 sm:px-6">Manual approach</div>
              <div className="px-2 py-3" aria-hidden />
              <div className="px-4 py-3 sm:px-6">
                With <ComplAIBrandLink inheritWeight />
              </div>
            </div>
            {WHY_COMPLAI_VS_MANUAL.map((row) => (
              <div
                key={row.manual}
                className="grid grid-cols-[1fr_auto_1fr] border-t border-white/10 bg-scrut-navy-light/50"
              >
                <div className="flex items-start gap-2 px-4 py-4 text-sm text-zinc-400 sm:px-6">
                ...
                <div className="flex items-start gap-2 px-4 py-4 text-sm text-zinc-300 sm:px-6">
      ...
      <section id="stages" className="scroll-mt-24 border-t border-white/10 bg-marketing-surface-alt py-16 sm:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-3xl font-bold tracking-tight text-zinc-100 sm:text-4xl">
            ...
            <p className="mt-4 text-lg text-zinc-400">
              Whether you need first audit readiness or enterprise-scale GRC,{' '}
              <ComplAIBrandLink inheritWeight /> adapts to where you are today.
            </p>
          </div>
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-3">
            {WHY_COMPLAI_STAGES.map((stage) => (
              <Link
                key={stage.title}
                href={stage.href}
                className="group flex flex-col rounded-2xl border border-white/10 bg-scrut-navy-light p-6 transition-all hover:border-scrut-teal/30 hover:shadow-md sm:p-8"
              >
                <h3 className="text-lg font-bold text-zinc-100 group-hover:text-scrut-blue">
                  {stage.title}
                </h3>
