...
import { MAX_EVIDENCE_FILE_BYTES, EVIDENCE_CONTEXT_LABELS } from '@/lib/evidence/constants';
...
export function EvidenceUploadPanel({
  ...
  const upload = async (file: File) => {
    ...
    if (context === 'issues' && issueId) formData.append('issueId', issueId);
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-slate-500">
        Upload policies, screenshots, audit artifacts. Max {MAX_EVIDENCE_FILE_BYTES / (1024 * 1024)} MB.
      </p>
      ...
      <div className="space-y-3">
        <div>
          ...
          <input
            ...
            placeholder="e.g. Privacy charter signed by CEO"
        ...
        {context === 'issues' && issues.length > 0 && (
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Link to issue (optional)</label>
            <select
              value={issueId}
            ...
            >
              <div className="flex min-w-0 gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-slate-900 truncate">{item.originalName}</p>
                  {item.description && (
