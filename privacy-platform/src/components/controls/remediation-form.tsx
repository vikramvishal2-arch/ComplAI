...
import type { RemediationAction, RemediationActionStatus } from '@/lib/types';
...
export function RemediationForm({ actions, onChange }: RemediationFormProps) {
  const addAction = () => {
    onChange([
      ...actions,
      {
    ...
    return (
      <div className="rounded-lg border border-dashed border-slate-300 p-8 text-center">
        ...
        <p className="mt-1 text-xs text-slate-500">
          Add steps to close privacy gaps for this control.
        </p>
        <button
          type="button"
          onClick={addAction}
  ...
  return (
    <div className="space-y-4">
      {actions.map((action, index) => (
        <div key={action.id} className="rounded-xl border border-slate-200 bg-slate-50/50 p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Publish privacy program charter"
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              ...
              <select
                value={action.status}
                onChange={(e) =>
                  updateAction(action.id, { status: e.target.value as RemediationActionStatus })
                }
                className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm bg-white"
              >
                {(Object.keys(REMEDIATION_STATUS_LABELS) as RemediationActionStatus[]).map((s) => (
                  <option key={s} value={s}>{REMEDIATION_STATUS_LABELS[s]}</option>
                ))}
              </select>
            </div>
            <div>
